...
if (isset($_POST['submit'])) {
	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			$sql = "INSERT INTO users (username, email,phone, d, password)
					VALUES ('$name', '$email', '$phone', '$d', '$password')";
			$result = mysqli_query($conn, $sql);
			if ($result) {
...
<body>
	<div class="container">
		<form action="" method="POST" class="login-email">
			...
			<div class="input-group">
				<input type="text" placeholder="Address" name="d" value="<?php echo $d; ?>" required>
			</div>
			<div class="input-group">
